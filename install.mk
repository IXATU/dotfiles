...
# Serena is a pinned, isolated uv tool. It is never installed into the shared
...
install-taplo:
	@bash $(DOTFILES_DIR)/scripts/install-taplo.sh $(if $(filter 1 true yes on,$(DRY_RUN)),--dry-run,)

# Add only the Serena user MCP through Claude's supported CLI. Existing Serena
...
# Non-mutating SOPS/Age/Chezmoi secrets readiness check; never prints values.
secrets-check:
	@bash $(DOTFILES_DIR)/scripts/secrets-check.sh
